	...
	// json_object_set_new(rootJ, "notes", json_float(pattern));
	json_t* notes_json_array = json_array();
	for (int notes_index=0; notes_index < 256; notes_index++) {
		json_array_append_new(notes_json_array , json_real(pattern[notes_index]));
	}	
